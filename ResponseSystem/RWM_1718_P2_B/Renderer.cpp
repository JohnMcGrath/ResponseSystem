...
//draw a rect in pixel coordinates
void Renderer::drawHollowRect(const Rect& r, const Colour& c) {
	SDL_SetRenderDrawColor(sdl_renderer, c.r, c.g, c.b, c.a);
	SDL_Rect sr;
	sr.h = (int)r.size.h;
	sr.w = (int)r.size.w;
...
//draw a rectin world coordinates
void Renderer::drawWorldHollowRect(const Rect &r, const Colour &c)
{
	drawHollowRect(worldToScreen(r), c);
}


//draw an image in pixel coordinates
void Renderer::drawImage(SDL_Texture* t) {

	// clear the screen
	SDL_RenderClear(sdl_renderer);

	//Copy to the renderer
	SDL_RenderCopy(sdl_renderer, t, nullptr, nullptr);
